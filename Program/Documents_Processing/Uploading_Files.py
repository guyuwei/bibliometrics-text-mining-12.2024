...
import bibtexparser
import re
from Documents_Processing.Data_Validator import validate_and_display_data_info
...
def Extract_Info_From_TXT(text):
    ...
    for line in lines:
        ...
        if line.startswith("AB "):
            ...
            current_field = "作者"
            # 如果已经有作者信息，用分号分隔多个作者
            if "作者" in current_info:
                current_content = [current_info["作者"] + ";" + line[3:].strip()]
            else:
                current_content = [line[3:].strip()]
            ...
            if current_field:
                current_info[current_field] = " ".join(current_content).strip()
            current_field = "作者全名"
            # 如果已经有作者全名信息，用分号分隔多个作者
            if "作者全名" in current_info:
                current_content = [current_info["作者全名"] + ";" + line[3:].strip()]
            else:
                current_content = [line[3:].strip()]
        ...
        #CA团体作者，CT会议标题，CY会议日期，CL，会议地点，CR 引用的参考文献，C1作者地址
            ...
            if current_field:
                current_info[current_field] = " ".join(current_content).strip()
            current_field = "国家"  # 修改为国家字段
            # 如果已经有地址信息，用分号分隔多个地址
            if "国家" in current_info:
                current_content = [current_info["国家"] + ";" + line[3:].strip()]
            else:
                current_content = [line[3:].strip()]
        ...
        #DA 生成此报告的日期，DE 作者关键词，DI 数字对象标识符 (DOI)，D2 书籍的数字对象标识符 (DOI)，DT 文献类型
            ...
            if current_field:
                current_info[current_field] = " ".join(current_content).strip()
            current_field = "关键词"  # 修改为关键词字段
            # 如果有关键词信息，用分号分隔多个关键词
            if "关键词" in current_info:
                current_content = [current_info["关键词"] + ";" + line[3:].strip()]
            else:
                current_content = [line[3:].strip()]
        ...
        #HO 会议主办方,ID Keywords Plus®,IS 期,JI ISO 来源文献名称缩写,J9 长度为 29 个字符的来源文献名称缩写
            ...
            # 如果有关键词信息，用分号分隔多个关键词
            if "Keywords Plus®" in current_info:
                current_content = [current_info["Keywords Plus®"] + ";" + line[3:].strip()]
            else:
                current_content = [line[3:].strip()]
        ...
        # 添加引用次数的别名映射
            ...
            if current_field:
                current_info[current_field] = " ".join(current_content).strip()
            current_field = '核心合集的被引频次计数'  # 添加引用次数字段
            current_content = [line[3:].strip()]
    ...
    # 确保返回有效的DataFrame
    if not info_list:
        return Filename, Versionnumber, pd.DataFrame()
    
    try:
        df = pd.DataFrame(info_list)
        return Filename, Versionnumber, df
    except Exception as e:
        st.error(f"创建DataFrame时出错: {str(e)}")
        return Filename, Versionnumber, pd.DataFrame()
...
def Load_TXT(uploaded_file):
    try:
        if uploaded_file.name.endswith('.txt'):
            ...
            filename, versionnumber, df = Extract_Info_From_TXT(data)
            
            # 检查返回的DataFrame是否有效
            if df is None or df.empty:
                st.warning("文件解析成功，但未找到有效的文献数据。请检查文件格式。")
                return pd.DataFrame()
            ...
            # 添加数据验证（静默验证，不显示信息）
            validation_result = validate_and_display_data_info(df)
            
            return df
        else:
            st.error("请上传.txt格式的文件")
            return pd.DataFrame()
...
# 导入增强的WOS解析器
try:
    from .Enhanced_WOS_Parser import load_wos_file_enhanced, get_wos_field_documentation
    ENHANCED_PARSER_AVAILABLE = True
except ImportError:
    ENHANCED_PARSER_AVAILABLE = False

@st.cache_data
def Load_TXT_Enhanced(uploaded_file):
    """
    使用增强的WOS解析器加载文件
    """
    if ENHANCED_PARSER_AVAILABLE:
        return load_wos_file_enhanced(uploaded_file)
    else:
        # 回退到原始解析器
...
def Load_Refine(uploaded_file):
    try:
        if uploaded_file.name.endswith('.xlsx'):
            ...
            st.error("请上传.xlsx格式的精炼文件")
            return pd.DataFrame()
            
    except Exception as e:
        st.error(f"处理精炼文件时出错: {str(e)}")
        return pd.DataFrame()


def parse_yunong_txt(file_path):
    """
    Parse the yunong.txt file to extract bibliographic data.
    """
    data = {
        'authors': [],
        'full_authors': [],
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        for line in lines:
            if line.startswith('AU '):
                data['authors'].append(line[3:].strip())
            elif line.startswith('AF '):
                data['full_authors'].append(line[3:].strip())
            elif line.startswith('TI '):
                data['titles'].append(line[3:].strip())
    
    return data

# Example usage
# data = parse_yunong_txt('/Users/gyw/Desktop/Project/2025/zyj_text_mining/000开发用/yunong.txt')
# print(data)
...
def convert_to_dataframe(parsed_data):
    """
    Convert parsed bibliographic data into a DataFrame.
    """
    df = pd.DataFrame({
        'Authors': parsed_data['authors'],
        'Full Authors': parsed_data['full_authors'],
        'Titles': parsed_data['titles']
    })
    return df

# Example usage
# parsed_data = parse_yunong_txt('/Users/gyw/Desktop/Project/2025/zyj_text_mining/000开发用/yunong.txt')
